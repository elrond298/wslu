...
@test "wslupath - No parameter" {
  run out/wslupath
  [ "${lines[0]}" = "wslupath [-dO] PATH" ]
  [ "${lines[1]}" = "wslupath -r [-dO] SHELL_FOLDER" ]
  [ "${lines[2]}" = "wslupath [-dO] LOCATION" ]
  [ "${lines[3]}" = "wslupath [-h|-v|-R]" ]
  [ "$status" -eq 20 ]
}
...
@test "wslupath - Help" {
  ...
  [ "${lines[1]}" = "Usage: wslupath [-dO] PATH" ]
  [ "${lines[2]}" = "wslupath -r [-dO] SHELL_FOLDER" ]
  [ "${lines[3]}" = "wslupath [-dO] LOCATION" ]
  [ "${lines[4]}" = "wslupath [-h|-v|-R]" ]
}
